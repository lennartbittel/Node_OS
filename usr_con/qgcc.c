...
int send_c(qsys *self,int* resval,int person)
{
	//self->operations[self->cur]=malloc(sizeof(ops));
	self->operations[self->cur].task=cmd_sendc;
	self->operations[self->cur].first=find_item(self->c_res,resval,self->icr);
	self->operations[self->cur].second=person;
	return (self->cur)++;
}
int recv_c(qsys *self,int* resval,int person)
{

	self->operations[self->cur].task=cmd_recvc;
	self->operations[self->cur].first=self->icr;
	self->operations[self->cur].second=person;
	self->c_res[(self->icr)++]=resval;
	return (self->cur)++;
}
...
int qkd()
{
	...
	int bob=0;
	srand(time(NULL));
	unsigned int a=rand()%10000;a=360;
	qsys sysA;init_sys(&sysA,alice,a);
	qsys sysB;init_sys(&sysB,bob,a);
	int resa[10];
	int resb[10];
	int resba[10];
	int result;
	for(int i=0; i<4;++i)
	{
		...
		apply_2gate(&sysA,cmd_cnot,q2,q1);
		send_q(&sysA,q1,bob);
		meas_q(&sysA,q2,&resa[i],remove);
		send_c(&sysA,&resa[i],bob);


		int qb=recv_q(&sysB,alice);
		meas_q(&sysB,qb,&resb[i],remove);
		recv_c(&sysB,&resba[i],alice);
	}
	printf("Alice:\n");
	result=perform_sys(&sysA,10,0);
	printf("Bob:\n");
	result=perform_sys(&sysB,10,0);
