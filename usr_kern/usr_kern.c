...
{
  char *argv[] = { "/media/lennart/UserData/Node_OS/usr_kern/test", "help!", NULL };
static char* envp[] = {
    "HOME=/",
    "TERM=linux",
...
static int dev_open1(struct inode *inodep, struct file *filep){
   ...
   printk("starting prog\n");
   
   i=umh_test();
   getnstimeofday64(&t2);
   
   printk("res :%d, time: %ld\n",i,t2.tv_nsec-t1.tv_nsec);


   i=umh_test2();
   getnstimeofday64(&t2);
   
   printk("res2 :%d, time: %ld\n",i,t2.tv_nsec-t1.tv_nsec);
...
static ssize_t dev_read(struct file *filep, char *buffer, size_t len, loff_t *offset){
   ...
   strcpy(message,"This is something");
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
   printk(KERN_INFO "Has written into it\n");
   return len;
}
